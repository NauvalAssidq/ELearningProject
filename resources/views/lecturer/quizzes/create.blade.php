...
@section('title', 'Buat Kuis - ' . $module->title)
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="quizBuilder()">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-text-main">Buat Kuis Baru</h1>
        <p class="text-text-muted mt-1">{{ $module->title }}</p>
    </div>

    <form action="{{ route('lecturer.quizzes.store', $module) }}" method="POST" class="space-y-8">
        @csrf
        ...
        <div class="bg-white border border-border p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="lesson_id" required class="w-full border border-border p-3 focus:outline-none focus:border-black">
                    ...
                    @if($lessons->isEmpty())
                        <p class="text-xs text-yellow-600 mt-1">Semua pelajaran sudah memiliki kuis.</p>
                    @endif
                </div>
                
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider mb-2">Batas Waktu (Menit)</label>
                    <input type="number" name="time_limit" value="15" min="1" max="180" required
            ...
            <div>
                ...
                <input type="text" name="title" required placeholder="Contoh: Kuis Evaluasi Pelajaran 1"
                    class="w-full border border-border p-3 focus:outline-none focus:border-black">
            </div>
            
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider mb-2">Deskripsi (Opsional)</label>
                <textarea name="description" rows="2" placeholder="Instruksi pengerjaan..."
                    class="w-full border border-border p-3 focus:outline-none focus:border-black"></textarea>
            </div>
        </div>
