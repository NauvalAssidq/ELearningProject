...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <div>
        ...
        <h1 class="text-3xl font-bold text-text-main">Tambah Pelajaran</h1>
        <p class="text-text-muted mt-1">Buat pelajaran baru untuk modul {{ $module->title }}</p>
    </div>

    <div class="bg-white border border-border p-8">
        <form action="{{ route('lecturer.lessons.store', $module) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="space-y-4" x-data="{ 
                ...
                <div class="flex justify-between items-center">
                    <label class="block text-xs font-bold uppercase tracking-wider text-text-main">Media Pembelajaran (Opsional)</label>
            ...
            <div class="flex items-end">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_preview" id="is_preview" value="1" {{ old('is_preview') ? 'checked' : '' }} class="w-4 h-4 border-border rounded text-black focus:ring-black">
